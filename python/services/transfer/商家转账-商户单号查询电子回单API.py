...
class QueryElectronicReceiptByMerchantOrderNo:
    """商家转账-商户单号查询电子回单实现类"""

    def __init__(self):
        self.host = "https://api.mch.weixin.qq.com"
        self.path = "/v3/fund-app/mch-transfer/elecsign/out-bill-no/{out_bill_no}"
        self.method = "GET"
    ...
    def parse_response(
        ...
        """
        返回参数说明：
        - state string: 申请单状态，可能值：GENERATING（生成中）、FINISHED（已完成）、FAILED（已失败）
        - create_time string(32): 申请单创建时间，格式为yyyy-MM-DDThh:mm:ss+TIMEZONE
        - update_time string(32): 申请单更新时间，格式为yyyy-MM-DDThh:mm:ss+TIMEZONE
        - hash_type string: 回单文件摘要类型（仅FINISHED状态返回），可选值：SHA256/SM3
        - hash_value string(512): 回单文件摘要值（仅FINISHED状态返回），需与下载文件实际摘要值对比一致性
        - download_url string(3000): 回单文件下载地址（仅FINISHED状态返回），有效期10分钟
        ...
        【state 申请单状态】处理建议:
            - GENERATING:
                # 电子回单生成中，建议查询间隔不低于2分钟
            - FINISHED:
                # 立即通过download_url下载回单文件，需校验hash_value与文件实际摘要值的一致性
                # 下载地址过期后需重新调用本接口获取（无需重新申请）
            - FAILED:
                # 停止重试查询，根据返回的失败原因进行处理
        """
