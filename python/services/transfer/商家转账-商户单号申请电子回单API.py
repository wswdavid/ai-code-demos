...
class ApplyElectronicReceiptByMerchantOrderNo:
    """商家转账-商户单号申请电子回单实现类"""

    def __init__(self):
        self.host = "https://api.mch.weixin.qq.com"
        self.path = "/v3/fund-app/mch-transfer/elecsign/out-bill-no"
        self.method = "POST"
    ...
    def run(self):
        ...
        # 发送请求
        http_response = self.send_request(body=body, path_params=path_params)
    ...
    def parse_response(
        ...
        返回参数说明：
        - state string: 申请单状态
        - create_time string(32): 申请单创建时间，按照使用rfc3339所定义的格式，格式为yyyy-MM-DDThh:mm:ss+TIMEZONE
        ...
        【state 申请单状态】处理建议:
            ...
            - FINISHED:
                # 表示当前电子回单已处理完成
            - FAILED:
                # 表示当前电子回单生成失败，需检查失败原因字段获取具体失败信息

        注意事项：
        1. 仅支持成功转账且6个月内的单据申请回单
        2. 回单申请后有效期为90天，过期需重新申请
        3. 受理成功不保证回单生成成功，需通过查单接口确认最终结果
        """
