...
class CreateTransfer(WeChatPayBase):
    ...
    def handle_http_status(self, status_code, result):
        ...
        match status_code:
            ...
            case 429:
                logger.warning(
                    f"请求频率超限，使用原商户订单号重试，商户单号: {out_bill_no}"
                )
                return (
                    "retry",
            ...
            case code if code in {500, 502, 503, 504}:
                if error_code in RETRIABLE_BIZ_CODES:
                    logger.warning(
                        f"服务端错误(可重试)，错误码: {error_code}，商户单号: {out_bill_no}"
                    )
                    return (
                        "retry",
                    ...
                    logger.error(
                        f"服务端错误(不可重试)，错误码: {error_code}，商户单号: {out_bill_no}"
                    )
                    return (
                        "error",
            ...
            case _:
                if error_code in RETRIABLE_BIZ_CODES:
                    logger.warning(
                        f"业务错误(可重试)，错误码: {error_code}，商户单号: {out_bill_no}"
                    )
                    return (
                        "retry",
                    ...
                    logger.error(
                        f"业务错误(不可重试)，错误码: {error_code}，商户单号: {out_bill_no}"
                    )
    ...
    def handle_transfer_result(self, data: dict):
        ...
        FAIL_REASON_HANDLERS = {
            ...
            "NAME_NOT_CORRECT": {
                ...
                "action": "verify_name",
                "retry": False,
                "actions": ["核实用户姓名信息", "通知用户更正姓名", "更新用户信息"],
            },
        }
        ...
        match state:
            ...
            case state if state in FINAL_STATES:
                if state == "SUCCESS":
                    ...
                    handler = FAIL_REASON_HANDLERS.get(
                        fail_reason,
                        {
                            "level": "error",
                            "message": "未知失败原因",
    ...
    def create_transfer_order(
        ...
        status_code, result = self._make_request(
            self.api_config["method"], self.api_config["path"], transfer_data
        )
